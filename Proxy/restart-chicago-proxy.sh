...
JAR_PATH="/run/media/dakto/minecraft/Proxy/velocity.jar"
...
while true; do
    ...
    if [ "$TIME_NOW" = "15:15" ] && [ "$LAST_RUN" != "$TIME_NOW" ]; then
        ...
        screen -S "$SCREEN_NAME" -X stuff "end$(printf \\r)"
        echo "Sent 'stop' command to Minecraft Proxy."
        ...
        for i in {1..30}; do
            ...
            echo "Waiting for proxy to stop... ($i/30)"
            sleep 1
        done

        # Start Minecraft server in a new screen session
        echo "Starting Minecraft Proxy in screen session '$SCREEN_NAME'..."
